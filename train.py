...
def fit_model(dsm: DeepSpeakerModel, working_dir: str, max_length: int = NUM_FRAMES, epochs=100, steps_per_epoch=1000):
	batcher = LazyTripletBatcher(working_dir, max_length, dsm)
	# build small test set.
	test_batches = []
	for _ in tqdm(range(200), desc='Build test set'):
		batch = batcher.get_batch_test(BATCH_SIZE)
...
def fit_model_softmax(dsm: DeepSpeakerModel, kx_train, ky_train, kx_test, ky_test,
	...
	dsm.m.fit(x=kx_train, y=ky_train, batch_size=batch_size,
...
def start_training(working_dir, pre_training_phase=True, epochs=100, steps_per_epoch=1000):
	...
	if pre_training_phase:
		...
		fit_model(dsm, working_dir, NUM_FRAMES, epochs, steps_per_epoch)
