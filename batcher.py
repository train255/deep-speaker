...
class LazyTripletBatcher:
		def __init__(self, working_dir: str, max_length: int, model: DeepSpeakerModel):
				...
				print("self.sp_to_utt_train", self.sp_to_utt_train)
				print("len self.sp_to_utt_train", len(self.sp_to_utt_train))
				self.sp_to_utt_test = train_test_sp_to_utt(self.audio, is_test=True)
				print("self.sp_to_utt_test", self.sp_to_utt_test)
				print("len self.sp_to_utt_test", len(self.sp_to_utt_test))
		...
		def update_triplets_history(self):
				...
				for speaker_id in selected_speakers:
						...
						if len(train_utterances) > 0:
							...
							print("ONE AUDIO speaker_id", speaker_id)
				if len(embeddings_utterances) > 0:
		...
		def get_random_batch(self, batch_size, is_test=False):
				...
				if len(speakers) >= size_rd:
					...
					print("get random batch", speakers)
					return None
